...
parameters {
  ### Fixed Effects: Intercepts
    vector[N_VL] alpha;       		    ## Virus Lineage 1            
    
  ### Fixed Effects: Slopes
    vector[N_VL] beta_Day;          	    ## Day      		     
    vector[N_VL] beta_LD;                   ## LD       
    vector[N_VL] beta_Temp;                 ## Temp      
    vector[N_VL] beta_Temp_X_Day;	    ## Temp x Day
...
transformed parameters {
  ...
  ### Linear Predictor
     ...
     Day[i] +  					  ## across days
     (beta_LD[VL[i]] +		 	          ## Log Dose slope for each virus lineage
     ...
     beta_Temp[VL[i]]) *			  ## Temperature slope for each Virus Lineage
     Temp[i] +					  ## across Temperature
     beta_Temp_X_Day[VL[i]] * Temp_X_Day[i];      ## Temp_Day Continuous Interaction
